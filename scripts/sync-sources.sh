...
if [ ! -e "$patchsets_info_file" ] ; then
  ...
  vnc_changes=$(cat $patchsets_info_file | jq -r '.[] | select(.project == "tungsten/contrail-vnc") | .project + " " + .ref + " " + .branch')
  if [[ -n "$vnc_changes" ]] ; then
    ...
    rm -rf contrail-vnc
    cmd="git clone --depth=1 --single-branch -b $vnc_branch ${GERRIT_URL}tungsten/contrail-vnc"
    echo "INFO: $cmd"
    eval "$cmd" || {
    ...
    echo "$vnc_changes" | while read project ref branch; do
      cmd="git fetch ${GERRIT_URL}tungsten/contrail-vnc $ref && git cherry-pick FETCH_HEAD "
...
if [ -e "$patchsets_info_file" ] ; then
  ...
  cat $patchsets_info_file | jq -r '.[] | select(.project != "tungsten/contrail-vnc") | .project + " " + .ref' | while read project ref; do
